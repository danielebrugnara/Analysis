...
Interpolation::Interpolation(std::string input_file):
        spline(nullptr) {
    if (!ReadFile(input_file)) throw std::runtime_error("Unable to generate interpolation from text file\n");
}

Interpolation::Interpolation(TFile *file):
        spline(nullptr){
    if (!(file->IsOpen())) throw std::runtime_error("Interpolation file pointer non initialized\n");
