...
void ReactionReconstruction2body<T>::Set_E(const T & E) {//Checked!
    ...
    T E_free = this->p1.Get_E()+this->p2.Get_M()-E;

    fixed.Set_E_Theta(  E,
                         acosl(
                            (this->p2.Get_M2()+free.Get_M2()-this->p1.Get_M2()-fixed.Get_M2()-2* this->p2.Get_M()*(E_free)+2*this->p1.Get_E()*E)/
                                            sqrt(4*(this->p1.Get_E()*this->p1.Get_E()-this->p1.Get_M2())*(E*E-fixed.Get_M2()))
                         ));

    free.Set_E_Theta_Phi(  E_free,
                           acosl(
                           (this->p2.Get_M2()+fixed.Get_M2()-this->p1.Get_M2()-free.Get_M2()-2* this->p2.Get_M()*(E)+2*this->p1.Get_E()*E_free)/
                           sqrt(4*(this->p1.Get_E()*this->p1.Get_E()-this->p1.Get_M2())*(E_free*E_free-free.Get_M2()))),
...
T ReactionReconstruction2body<T>::Set_E_cm(const T & E) {
    ...
    T sqrt_s = this->p1.Get_E_cm() + this->p2.Get_E_cm();
    auto& fixed = GetFixedFragment();
    auto& free = GetFreeFragment();
...
T ReactionReconstruction2body<T>::Set_Ek_cm(const T &Ek) {
    if(GetFixedFragment()==GetExFixedFragment()) {
        ...
        T sqrt_s = this->p1.Get_E_cm() + this->p2.Get_E_cm();
        T s = powl(this->p1.Get_E_cm() + this->p2.Get_E_cm(), 2);
        T ex = -sqrtl(free.Get_M2()+2.*Ek*sqrt_s) + sqrt_s - fixed.Get_M();
        GetExFreeFragment().Set_Ex(ex);
...
template<typename T>
void ReactionReconstruction2body<T>::Set_Theta(const T & Theta,
                                            const bool & chose_max_solution) {//Checked
    auto & fixed    = GetFixedFragment();
    auto & free     = GetFreeFragment();
    ...
    T sqroot =  cosine*cosine*p1_mag2 *
    ...
    T denominator = 2. * ( powl(E1+m2, 2)-p1_mag2*cosine*cosine);
    T e3 = 0;
...
void ReactionReconstruction2body<T>::Set_Theta_cm(const T & Theta) {//Not yet correct
    ...
    T en_fix = sqrtl(pf2+fixed.Get_M2());
    T en_free = sqrtl(pf2+free.Get_M2());
